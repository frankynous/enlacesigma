$(function() {
    var mediumPromise = new Promise(function(resolve) {
      ...
      var data = {
        rss: "https://medium.com/feed/@frankynous"
      };
      $.get(
        "https://api.rss2json.com/v1/api.json?rss_url=https%3A%2F%2Fmedium.com%2Ffeed%2F%40frankynous",
        data,
        function(response) {
          if (response.status == "ok") {
            ...
            $.each(response.items, function(k, item) {
              ...
              output += `<h5 class="card-title"><a href="${item.link}" target="blank">${
                item.title
              }</a></h5>`;
              ...
              output += `<a href="${
                item.link
              }" class="btn btn-outline-success" target="blank">Continua...</a>`;
              output += "</div></div>";
              return k < 10;
            });
